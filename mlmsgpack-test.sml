structure PackTest = struct
  ...
  val tests = [
    ...
    ("packString1", fn () => doPack packString "" = [0xa0]),
    ("packString2", fn () => doPack packString "0123456789012345678901234567890" = [0xbf, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30]),
    ("packString3", fn () => doPack packString "01234567890123456789012345678901" = [0xd9, 0x20, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31]),
    ("packString4", fn () => doPack packString (Byte.bytesToString (Word8Vector.tabulate (255, fn n => 0w1))) = [0xd9, 0xff] @ List.tabulate (255, fn n => 1)),
    ("packString5", fn () => doPack packString (Byte.bytesToString (Word8Vector.tabulate (256, fn n => 0w1))) = [0xda, 0x01, 0x00] @ List.tabulate (256, fn n => 1)),
    ("packString6", fn () => doPack packString (Byte.bytesToString (Word8Vector.tabulate (65535, fn n => 0w1))) = [0xda, 0xff, 0xff] @ List.tabulate (65535, fn n => 1)),
    ("packString7", fn () => doPack packString (Byte.bytesToString (Word8Vector.tabulate (65536, fn n => 0w1))) = [0xdb,  0x00, 0x01, 0x00, 0x00] @ List.tabulate (65536, fn n => 1)),

    ("packBytes1", fn () => doPack packBytes (Byte.stringToBytes "") = [0xc4, 0x00]),
    ("packBytes2", fn () => doPack packBytes (Byte.stringToBytes "0123456789012345678901234567890") = [0xc4, 0x1f, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30]),
    ("packBytes3", fn () => doPack packBytes (Byte.stringToBytes "01234567890123456789012345678901") = [0xc4, 0x20, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31]),
    ("packBytes4", fn () => doPack packBytes (Word8Vector.tabulate (255, fn n => 0w1)) = [0xc4, 0xff] @ List.tabulate (255, fn n => 1)),
    ("packBytes5", fn () => doPack packBytes (Word8Vector.tabulate (256, fn n => 0w1)) = [0xc5, 0x01, 0x00] @ List.tabulate (256, fn n => 1)),
    ("packBytes6", fn () => doPack packBytes (Word8Vector.tabulate (65535, fn n => 0w1)) = [0xc5, 0xff, 0xff] @ List.tabulate (65535, fn n => 1)),
    ("packBytes7", fn () => doPack packBytes (Word8Vector.tabulate (65536, fn n => 0w1)) = [0xc6,  0x00, 0x01, 0x00, 0x00] @ List.tabulate (65536, fn n => 1)),
